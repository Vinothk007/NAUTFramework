...
public class DynamicTestNG {
	...
	public static void main(String args[]) {
		...
		System.out.println("Delete   "+instance[0]);
		if (!instance[0].contains("Emu")) {
			...
			for (int inst=1; inst<=getThreadCount(); inst++) {
				XmlTest test = new XmlTest(suite);
				test.addParameter("instance", instance[0]);
				test.setClasses(Arrays.asList(cls));
				test.setName("Session " + instance[0]+inst);
			}
		}
